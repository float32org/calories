...
export async function optimizeCaloriesWithAI(
	...
	const weightInKg = unit === 'kg' ? currentWeight : currentWeight * 0.453592;
	const diffInLbs = unit === 'lbs' ? Math.abs(weightDiff) : Math.abs(weightDiff) * 2.20462;
...
Estimated weight in kg: ${weightInKg.toFixed(1)} kg
</user_data>
...
<calculation_steps>
1. Estimate TDEE using weight-based calculation (assume lightly active lifestyle)
2. Determine appropriate deficit/surplus (500-750 cal for sustainable progress)
3. Ensure calorie target is above minimum safe threshold
4. Calculate realistic timeline with buffer for real-world adherence
5. Provide brief, encouraging explanation
</calculation_steps>

<constraints>
- Minimum calories: ${weightInKg > 90 ? '1,400' : '1,200'}
- Target rate: ${diffInLbs > 30 ? '1-2 lbs/week initially, slowing over time' : '0.5-1.5 lbs/week'}
- Timeline should be a RANGE (e.g., "8-12 weeks") accounting for real-world variability
- Explanation should be 2-3 sentences, friendly and motivating
</constraints>`;
